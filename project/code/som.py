...
class SOM(object):
	...
	def initialize(self, shape):
		...
		if shape == '2dgrid':
			self.contents = grid(self.rows, self.cols, self.dim).to(self.device)
		elif shape == 'ball':
			self.contents = torch.normal(
				0, 0.2*torch.ones(self.rows, self.cols, self.dim)).to(self.device)
		...
		self.grid_used = torch.zeros(self.rows, self.cols).long().to(self.device)
...
class ParallelBatchSOM(SOM):
	...
	def update(self, x, sigma, weighted=False):
		...
		batch_num = torch.LongTensor(range(self.batches)).to(self.device)
		min_idx = batch_num.view(-1,1) * self.rows*self.cols + min_idx

		# Compute the frequency with which each node is the BMU
		freq_data = torch.zeros(self.batches, self.rows*self.cols).to(self.device)
		freq_data.view(-1).index_add_(0, min_idx.view(-1), torch.ones(x.shape[:2]).to(self.device).view(-1))
		...
		sum_data = torch.zeros(self.batches, self.rows*self.cols, self.dim).to(self.device)
...
class BatchSOM(SOM):
	...
	def update(self, x, sigma, weighted=False):
		...
		# Compute the frequency with which each node is the BMU
		freq_data = torch.zeros(self.rows*self.cols).to(self.device)
		freq_data.index_add_(0, min_idx, torch.ones(x.shape[0]).to(self.device))

		# Store the update frequency for each node
		self.grid_used += (freq_data != 0).view(self.rows, self.cols).long()

		# Compute aggregate data values for each neighborhood
		sum_data = torch.zeros(self.rows*self.cols, self.dim).to(self.device)
